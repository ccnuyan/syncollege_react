...
At this point, you can delete everything up until `queryType` in `./schema/schema.js`.
...
const {nodeInterface, nodeField} = nodeDefinitions(
  (globalId) => {
    const {type, id} = fromGlobalId(globalId); // fromGlobalId: get type and id of an object.
    if (type === 'Game') {
      return getGame(id);
  ...
  }, // We need only provide a way for Relay to map from a global ID to the object it points to.
  (obj) => {
  ...
  } // We need only provide a way for Relay to map from an object to the GraphQL type associated with that object.
);
```
  ...
  renderGameBoard() {
    return this.props.game.hidingSpots.edges.map(edge => {
      return (
        <div
          key={edge.node.id}
          onTouchTap={this._handleHidingSpotClick.bind(this, edge.node)}
